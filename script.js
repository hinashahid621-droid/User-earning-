...
function logout() { currentUser = null; loginForm.reset(); showView('login-view'); }
...
// --- WITHDRAWAL FUNCTION ---
withdrawForm.addEventListener('submit', async (e) => { e.preventDefault(); const amount = parseInt(document.getElementById('withdraw-amount').value); const method = document.getElementById('withdraw-method').value; const number = document.getElementById('withdraw-number').value; if (amount < 500) return alert("Minimum withdrawal is 500 PKR."); if (amount > currentUser.points) return alert("You don't have enough balance."); if (!method || !number) return alert("Please fill all fields."); const withdrawButton = withdrawForm.querySelector('button'); withdrawButton.disabled = true; const { error: reqError } = await supabaseClient.from('withdrawals').insert([{ user_id: currentUser.id, amount, method, account_number: number, status: 'pending' }]); if (reqError) { alert("Error submitting request: " + reqError.message); withdrawButton.disabled = false; return; } const newPoints = currentUser.points - amount; const { error: userError } = await supabaseClient.from('users').update({ points: newPoints }).eq('id', currentUser.id); if (userError) { alert("Error updating your balance. Please contact support."); withdrawButton.disabled = false; return; } currentUser.points = newPoints; if (currentUser.referred_by) { const { data: referrer } = await supabaseClient.from('users').select('id, points').eq('referral_code', currentUser.referred_by).single(); if (referrer) { await supabaseClient.from('users').update({ points: referrer.points + 20 }).eq('id', referrer.id); } } alert(`Withdrawal request for ${amount} PKR submitted successfully!`); updateDashboard(); withdrawForm.reset(); showView('dashboard'); withdrawButton.disabled = false; });
